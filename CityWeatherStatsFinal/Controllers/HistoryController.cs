...
namespace CityWeatherStatsFinal.Controllers
{
    ...
    public class HistoryController : Controller
    {
        ...
        public JsonResult GetDailyWeather(HistoricalWeatherParameter p)
        {
            ...
            // If dates between start and end are longer than 3 months, pull from 
            ...
            applyFilterOperator(p.filters, ref dailyWeatherData);
        ...
        private int pullHistoricalWeatherFromNOAA(CityWeatherStatsFinal.Models.City md)
        {
            bool init = true;
            int Rc = 0;
            ...
            while (getStartEndTimes(ref startDate, ref endDate, ref init, md))
            {
                ...
                Rc = getNOAADailiesForCityTimePeriod(startDate, endDate, md.cityid, dailyWeatherData);
        ...
        private int pullHistoricalDataFromBackend(CityWeatherStatsFinal.Models.City md)
        {
            int Rc = 0;  
            dailyWeatherData = dailyWeatherContext.CityDailyWeather.Where(x => x.cityid == md.cityid && x.date >= md.mindate && x.date <= md.maxdate).OrderBy(x=> x.date).ToList();
            return Rc; 
        }
    }
}
